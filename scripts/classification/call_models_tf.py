...
def build_model_tf(name_model, learning_rate, backbone_model='', num_classes=1,
                   include_top=False, train_backbone=False, trainable_layers=-1,
                   optimizer='adam', loss='categorical_crossentropy',
                   metrics=["accuracy", tf.keras.metrics.Precision(), tf.keras.metrics.Recall()], preprocess_input=[],
                   data_augmentation=[], input_size=(224, 224, 3)):
    # load the backbone
    if backbone_model != '':
        base_model, input_size = load_pretrained_model(backbone_model)
...
def resize_and_rescale(image, label, IMG_SIZE):
...
def tf_parse(x, y):
    ...
    x = tf.numpy_function(_parse, [x, y], [tf.float64, tf.float64])
    x.set_shape([256, 256, 3])
    x = preprocess_input(x)
    return x, y


def tf_dataset(x, y, batch=8, shuffle=False):
...
def load_data(data_dir, annotations_file='', backbone_model='',
    ...
    AUTOTUNE = tf.data.AUTOTUNE
    backbone_input_sizes = {'vgg16': (224, 244), 'vgg19': (224, 224),
                            'inception_v3': (299, 299), 'resnet50': (224, 224),
                            'resnet101': (224, 224), 'densenet121': (224, 224),
                            'mobilenet': (224, 224), 'xception': (299, 299)}
    if backbone_model:
        img_size = backbone_input_sizes[backbone_model]

    tf_dataset = tf.keras.utils.image_dataset_from_directory(data_dir,
                                                             shuffle=True,
                                                             batch_size=batch_size,
                                                             image_size=img_size)
    ...
    return tf_dataset, num_classes, img_size


def load_data_gen(data_dir, annotations_file='', backbone_model='',
                  img_size=(255, 255), batch_size=8, prediction_mode=False):
...
def evaluate_and_predict(model, directory_to_evaluate, results_directory,
    ...
    data_gen, _ = load_data_gen(directory_to_evaluate, backbone_model=backbone_model,
                                batch_size=8, prediction_mode=True)
        ...
        if list_files:
            ...
            if len(np.unique(predicts)) == 2:
                auc = daa.calculate_auc_and_roc(results_csv_file, dir_annotations_csv, output_name, plot=False,
                                                results_directory=results_directory, results_id=results_id,
                                                save_plot=True)
                print(f'AUC: {auc}')
        else:
...
def call_models(name_model, mode, data_dir=os.getcwd() + '/data/', validation_data_dir='',
                ...
                file_to_predic='', trainable_layers=-1, fine_tune_epochs=1, data_augmentation=[]):
    AUTOTUNE = tf.data.AUTOTUNE
    ...
    if mode == 'train' or mode == 'train_backbone':
        ...
        # Prepare the TF datasets

        train_dataset = load_data(train_data_dir, backbone_model=backbone_model,
                                  batch_size=batch_size)
        ...
        model = build_model_tf(name_model, backbone_model=backbone_model, num_classes=1)
        model.summary()
